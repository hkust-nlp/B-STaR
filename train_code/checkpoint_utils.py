...
def load_inference_checkpoint(
    ...
    if not torch.distributed.is_initialized():
        ...
        print("ckpt_world_size", ckpt_world_size)
...
def load_inference_checkpoint_mistral(
    checkpoint_path: str,
    model: torch.nn.Module,
) -> None:
    """Loads a checkpoint.
    ...
    if not torch.distributed.is_initialized():
        ...
        if ckpt_world_size == world_size:
            ...
            state_dict = merge_tp_checkpoint_mistral(all_state_dict)
            ...
            for key in full_state_dict:
                full_state_dict[key] = full_state_dict[key].cpu()
            state_dict = split_tp_checkpoint_mistral(full_state_dict, split_factor, split_idx)
...
def merge_tp_checkpoint_mistral(
    all_state_dict: list,
):
    state_dict = {}
...
def split_tp_checkpoint_mistral(
    full_state_dict: dict,
    split_factor: int,
    split_idx: int,
):
    ...
    if not skip_init:
        ...
        if sft_checkpoint_path is not None:
            ...
            if "Mistral-7B-v0.1" in checkpoint_path.parent.name:
                load_inference_checkpoint_mistral(sft_checkpoint_path, model)
...
def load_reward_model_from_sft_ckpt_deepseek(
    checkpoint_path: Path,
    sft_checkpoint_path: Optional[Path],
    device: torch.device,
    precision: torch.dtype,
...
def load_reward_model_from_sft_ckpt_llama3(
    checkpoint_path: Path,
    sft_checkpoint_path: Optional[Path],
    device: torch.device,
    precision: torch.dtype,
    ...
    #model.backbone_model.load_state_dict(model_state_dict, strict=False, assign=True)
    #model.backbone_model.load_state_dict(checkpoint, assign=True, strict=False)
