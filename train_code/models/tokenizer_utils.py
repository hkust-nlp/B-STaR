...
class AcceleraTokenizer:
    ...
    def __init__(
        ...
        if str(tokenizer).endswith(".model"):
            ...
            # print("self.hf_model.vocab_size", self.hf_model.vocab_size)

            # print("self.model_vocab_size", self.model_vocab_size)


            # print("len(self.hf_model)", len(self.hf_model))

            self.model_vocab_size = len(self.hf_model)
            ...
            self.pad_id: int = 128008
            self.unk_id: int = 128010
            self.bos_token = self.hf_model.bos_token
            self.eos_token = self.hf_model.eos_token

        
                   
        elif "deepseek-coder" in str(tokenizer):
            if AutoTokenizer is None:
                raise ImportError(
                    "transformers is not installed. Please install it to use AutoTokenizer."
                )
            self.hf_model = AutoTokenizer.from_pretrained(tokenizer)
            self.sp_model = None           
            self.model_vocab_size = self.hf_model.vocab_size
            self.n_words: int = self.model_vocab_size
            self.bos_id: int = self.hf_model.bos_token_id
            self.eos_id: int = self.hf_model.eos_token_id
        ...
        if "Llama-3" in str(tokenizer):
            self.pad_token = "<|reserved_special_token_4|>"
            self.unk_token = "<|reserved_special_token_5|>" 
